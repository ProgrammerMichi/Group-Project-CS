...
with col6:
    ...
    with col6_1:
        selmin_rating = st.number_input("Minimum Rating", min_value=0, max_value=10)

    with col6_2:
        selmax_rating = st.number_input("Maximum Rating", min_value=0, max_value=10, value = 10)
...
def findmovie():
    ...
    if keyword_check and selkeywords:
        keyword_ids = [str(keyword.id) for keyword in Instance.keywords.search(selkeywords)]
        search_parameters["with_keywords"] = ",".join(keyword_ids)
